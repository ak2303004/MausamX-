...
/**
 * Maps every period-select value to the number of days back in time.
 * Short-range values (≤ 30 days) are served by the fast USGS GeoJSON feed.
 * Extended values (> 30 days) MUST use the USGS FDSNWS event API.
 */
const RANGE_DAYS = {
  hour: 1 / 24,
  day: 1,
...
/**
 * Maximum number of results to request from FDSNWS for large time windows.
 * Keeps response size and render time manageable (API hard cap is 20 000).
 */
